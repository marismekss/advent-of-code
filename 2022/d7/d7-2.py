...
currentFreeDisk = 70000000 - root.size
neededDisk = 30000000 - currentFreeDisk


## Put all folder sizes in one list
folderSizes = []
for node in tree:
    if node.type == 'folder':
        folderSizes.append(node.size)


for i in sorted(folderSizes):
    if i > neededDisk:
        foundSize = i
        break

print('Result: ',foundSize)