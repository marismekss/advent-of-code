"""AoC day 3 part 2"""
#FILEPATH = ".2023/d3/source-test.txt"
FILEPATH = "./2023/d3/source-prod.txt"
        ...
        for c in range(num["idx_c"]-1, num["idx_c"] + num["length"]+1):
            ...
            if not out_of_bounds(r, c, max_r, max_c):
                if (grid.cell[r][c].isalpha() is False and
                    ...
                    # if char found as gear, in 2nd grid add number in that cell
                    if grid.cell[r][c] == '*':
                        if gear_grid.cell[r][c] == '.':
                            gear_grid.cell[r][c] = []
                        gear_grid.cell[r][c].append(num["number"])
...
def get_ratios(max_r, max_c):
    """Find gears and multiply numbers to get ratio"""
    for r in range(0, max_r):
        for c in range(0, max_c):
            cell_value = gear_grid.cell[r][c]
            if len(cell_value) > 1:
                res = int(cell_value[0]) * int(cell_value[1])
